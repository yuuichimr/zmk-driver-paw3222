...
| テント | 別途 | 別途 | 別途 | **ケース単体で角度可変** | — | — | — | **一体テント（角度はプリント時に選択）** |
...
- **Toucan**: 40mm Cirque トラックパッドとメモリ LCD。→ ベイに **35mm Cirque トラックパッド**（タップでクリック）を載せられるようにした。表示は電池寿命を優先して RGB LED に留めた。
...
1. **ボール・トラックパッド・エンコーダー・ブランクを差し替えられる「モジュールベイ」を左右両方に**
   ...
   PAW3222 ボール（25/19mm）・Cirque 35mm トラックパッド（I2C）・EC11 エンコーダー（押し込みはマトリクスの1キー）・ブランクを交換可能。
...
| ポインティング | PixArt PAW3222 + 25mm / 19mm ボール（3mm セラミックボール 3点支持）、または Cirque 35mm トラックパッド |
| 接続 | BLE（最大5台）+ USB-C、ZMK Studio |
| PCB | 2層 1.6mm、片側 140.3 × 96.5 mm、左右別基板 |
| ケース | 3Dプリント（サポート不要）、一体テント 9.5°、マグネット連結、片側 144.8 × 101.9 × 32.0 mm |
...
| `tsumugi_right_trackpad.uf2` | 右 | 右ベイ = 35mm トラックパッド（カーソル、タップでクリック） |
...
| Cirque TM035035（35mm トラックパッド） | 0〜2 | トラックパッド構成時。I2C モード（R1 抵抗を外す）で使用 |
